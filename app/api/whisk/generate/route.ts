...
// MASTER STYLE LOCK (applied to every Whisk image request)
const WHISK_MASTER_STYLE_PROMPT = `
Ultra-detailed miniature diorama scene, handcrafted scale model look.
Tiny human figures like realistic figurines, not life-size.
Extreme shallow depth of field, macro photography, tilt-shift effect.
The environment looks like a carefully built historical diorama, not a real photograph.

People look like miniature dolls with realistic proportions, soft facial expressions, subtle emotions.
Skin texture smooth but natural, no plastic shine.

Lighting: warm golden sunlight, soft cinematic lighting, global illumination, gentle shadows.
Color grading: nostalgic, warm, earthy tones, cinematic film look.
Background slightly blurred, foreground sharp like macro lens f/1.8.
Strong sense of scale: houses and people clearly miniature.

Materials: visible wood grain, clay roof texture, woven baskets, fabric fibers.
Highly detailed environment, calm atmosphere, peaceful daily life.

Camera angle: eye-level with miniature figures, close-up macro shot.
Photorealistic but clearly miniature diorama, handcrafted model realism.
No modern elements, no text, no watermark, no logo.
`.trim();
...
export async function POST(req: Request) {
  try {
    ...
    // LOCK: prepend master style to every prompt (ref and non-ref)
    const finalPrompt = `${WHISK_MASTER_STYLE_PROMPT}\n\nSCENE DETAILS:\n${prompt}`.trim();
